...
class Measure(Processor):
    ...
    def build_tagger(self):
        ...
        #pp = add_weight((digits + cross('-', ',') + digits + cross('-', ',') + digits + cross('-', ',') + digits + cross('-', ',') + digits + cross('-', ',') + digits), 0.001)
        #pp = add_weight((digits + cross('-', ',')).closure(4), 0.1)
        #fixcase = pp 

        tagger = insert('value: "') + (measure | annual) + insert('"')
        #tagger = insert('value: "') + (measure | annual | fixcase) + insert('"')
